...
| http.sink.success-codes                   | optional | Comma separated HTTP status codes considered as successful sink responses. Use [1-5]XX for groups and `!` for exclusions. The default is `2XX`. Cannot be set together with the legacy `http.sink.error.code` properties. |
| http.sink.retry-codes                     | optional | Comma separated HTTP status codes considered as retryable sink responses. Use [1-5]XX for groups and `!` for exclusions. The default is `500,503,504`. Cannot be set together with the legacy `http.sink.error.code` properties. |
| http.sink.ignored-response-codes          | optional | Comma separated HTTP status codes that should be treated as successful without retrying. Use [1-5]XX for groups and `!` for exclusions. Empty by default. Cannot be set together with legacy `http.sink.error.code.exclude`. |
...
If either legacy property is set together with `http.sink.success-codes` or `http.sink.retry-codes`, sink creation fails. Setting `http.sink.error.code.exclude` together with `http.sink.ignored-response-codes` also fails. Use one option set or the other. Existing jobs that only set the legacy properties keep the previous error-code behaviour. Prefer:
