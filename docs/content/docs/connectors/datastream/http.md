...
* [Apache HTTP Connector](#apache-http-connector)
  * [Working with HTTP sink Flink streaming API](#working-with-http-sink-flink-streaming-api)
    ...
    * [Retries and handling errors](#retries-and-handling-errors)
...
HTTP responses listed in `http.sink.success-codes` are treated as successful writes. Responses listed in
`http.sink.ignored-response-codes` are also treated as successful, but their response body is ignored. Any response code
that is not success, ignored, or retryable is fatal and fails the job.
